---
title: "Thí nghiệm 3 – Thời gian để ráo và chốt thông số vận hành"
researchDate: 2026-10-11
publishedDate: 2026-08-28
authors:
  - "Lê Quang Minh"
  - "Vũ Tuệ Phương"
summary: "Kế hoạch nhật ký ngày 11/10/2026. Chỉ chuyển sang hoàn thành sau khi bổ sung dữ liệu và minh chứng thực tế."
activityTypes:
  - "thi-nghiem"
  - "phan-tich"
status: "planned"
experimentIds:
  - "TN3"
systemVersion: "v0.6"
tags:
  - "TN3"
  - "để ráo điện cực"
  - "KCl"
  - "tham số vận hành"
images: []
youtubeIds: []
dataFiles: []
draft: true
---

> **Nhật ký được tạo trước theo lộ trình.** Nội dung dưới đây là kế hoạch thực hiện, không phải kết quả đã đạt. Sau buổi nghiên cứu, hãy thay các phần hướng dẫn bằng ghi chép thực tế, bổ sung ảnh/video/dữ liệu, đổi `status` và chỉ đặt `draft: false` khi đã kiểm tra.

## Mục tiêu

- Quan sát lượng dung dịch bám theo thời gian.
- Đánh giá ảnh hưởng đến phép đo kế tiếp nếu có thể.
- Chốt thời gian chờ, N và thời gian để ráo cho TN4.

## Cơ sở và nội dung dự kiến

Nguyên mẫu chưa có cơ chế rửa riêng; thời gian để ráo 5, 15, 30 và 60 giây được khảo sát để giảm lượng mẫu bám trước khi điện cực trở về KCl.

## Thiết bị và vật liệu

- [CẦN CẬP NHẬT] Liệt kê đúng thiết bị, vật liệu và phiên bản phần cứng/phần mềm thực sự sử dụng trong buổi này.
- [CẦN CẬP NHẬT] Ghi tình trạng hiệu chuẩn, nguồn điện, dung dịch và thiết bị tham chiếu nếu có.

## Các bước thực hiện

- [ ] Thử bốn mức thời gian.
- [ ] Ghi số giọt/quan sát bề mặt.
- [ ] Thực hiện chuyển mẫu nếu điều kiện cho phép.
- [ ] Không gọi bước để ráo là bước rửa.
- [ ] Khóa bộ tham số cuối cho TN4.

## Kết quả và quan sát

- [CẦN CẬP NHẬT] Mô tả những việc thực sự đã hoàn thành.
- [CẦN CẬP NHẬT] Ghi hiện tượng quan sát được, kể cả kết quả không như mong đợi.
- [CẦN CẬP NHẬT] Phân biệt rõ dữ liệu đo, nhận xét và suy luận.

## Số liệu

- [CẦN CẬP NHẬT] Quan sát dung dịch bám.
- [CẦN CẬP NHẬT] pH và E của phép đo kế tiếp.
- [CẦN CẬP NHẬT] Bộ tham số được lựa chọn.

## Vấn đề và sai số

- [CẦN CẬP NHẬT] Lỗi, giới hạn, sai số, can thiệp thủ công hoặc điều kiện khác với kế hoạch.
- [CẦN CẬP NHẬT] Không xóa dữ liệu bất thường; ghi lý do nếu loại khỏi phân tích.

## Điều chỉnh

- [CẦN CẬP NHẬT] Thay đổi về cơ khí, điện tử, thuật toán, tham số hoặc quy trình sau buổi nghiên cứu.
- [CẦN CẬP NHẬT] Ghi mã phiên bản/commit nếu thay đổi chương trình hoặc thiết kế.

## Minh chứng cần bổ sung

- [ ] Bảng 3C.
- [ ] Ảnh/video điện cực sau các mốc.
- [ ] Bảng bộ tham số cuối.
- [ ] Commit cấu hình nếu có.

## Công việc tiếp theo

Chuẩn bị kiểm tra vận hành định kỳ và độ tin cậy.
